...
The repository also records the post-implementation architecture correction that **browser, mobile and desktop are adapters over one shared AISE product/domain core**. No client becomes a second product authority.
...
| PROD-012 | PROD-011, PROD-011b | Browser verification, accessibility, responsive behavior and smoke tests | Automated browser checks pass on desktop/mobile breakpoints; no blocking console/runtime errors |
...
| PROD-016 | PROD-003 | Shared client adapter contract and conformance suite for browser, mobile and desktop | All three adapters consume the same semantic task/capability contract without alternate authority |
| PROD-017 | PROD-010, PROD-016 | Task-first next-best-action UX, cross-adapter journey parity and interaction polish | Core tasks are actionable without understanding AISE internals; semantic results remain equivalent across adapters |
| PROD-018 | PROD-017 | Competitive-parity hardening: capture friction, auditability, integrations, before/after and outcome loop | Simulated benchmark against major adjacent platforms passes defined product requirements |
| PROD-014 | PROD-012, PROD-013, PROD-016, PROD-017, PROD-018 | Install/release documentation and one-command demo bootstrap | Fresh evaluator follows docs and reaches the working product without repository archaeology |
...
Wave P5
  PROD-011   PROD-013   PROD-016

Wave P6
  PROD-011b   PROD-012   PROD-017
...
- representative mobile capture journey evidence;
- desktop adapter smoke evidence;
