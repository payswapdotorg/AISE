 ...
 * package's index re-exported the Node-only committed-fixture loader (plus
 * its `SolutionFixtureRecord`/`SolutionFixtureCorpus` types), so every
 * browser bundle that imported the barrel also bundled the loader — whose
 * module scope evaluates `join(import.meta.dir, "..")` from `node:path`
 * and crashed module evaluation in a plain browser (the bundler
 * externalizes the Node builtins).
 *
 * The seam fix: the loader (and its types) are exported ONLY through the
 * documented subpath `@aise/solution-contract/fixtures-loader` (package.json
 * exports map), and the barrel re-exports are gone. These tests pin BOTH
 * sides of that seam:
 *
...
import type { SolutionFixtureRecord, SolutionFixtureCorpus } from "./fixtures-loader";
...
describe("PROD-030 the documented fixtures-loader subpath still works (Node consumers)", () => {
  ...
  test("the deep import loads the non-empty committed corpus (typed round-trip)", () => {
    const corpus: SolutionFixtureCorpus = loadCommittedFixtures();
    expect(corpus.fixtures.length).toBeGreaterThan(0);
    for (const fixture of corpus.fixtures) {
      const record: SolutionFixtureRecord = fixture;
      expect(record.objectName.length).toBeGreaterThan(0);
      expect(["valid", "invalid", "version-mismatch"]).toContain(record.kind);
