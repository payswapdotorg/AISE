/**
 * `bun run gen:schemas` (PROD-016) — CLI entry for JSON Schema generation.
 *
 * Writes the deterministic schema set (see `scripts/lib/generate.ts`) into
 * `packages/adapter-contract/schemas/`. Regeneration on a clean tree must
 * produce a zero-byte diff; the byte-stability is additionally enforced by
 * a bun test. Committed schemas are consumed by the Android client (and any
 * other non-TypeScript consumer) without a TypeScript dependency.
 *
