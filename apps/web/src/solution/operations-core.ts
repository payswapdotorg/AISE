 ...
 * PATH of the interactive solution workspace — the BROWSER-SAFE CORE
 * (PROD-031's cut).
 *
 ...
 *    `submitIntent` (the agent decision controller below).
 *
 ...
 *
 * PROD-031 (the browser-safe cut): this module is the crypto-free core the
 * browser mount's chunk graph imports — the contract VALUES come from the
 * `@aise/solution-contract/browser` subpath (the barrel re-exports the
 * `node:crypto`-dependent identity derivations, which a plain-browser
 * bundle externalizes), and the two crypto-DEPENDENT helpers moved to the
 * Node-side facade `./operations`:
 *
 *  - `openWorkspace`'s engine fallback (the local
 *    `materializeBaselineState` baseline overlay) — HERE the baseline
 *    overlay is ALWAYS an injected, engine-computed `ProposedState`
 *    (`openWorkspace` requires it; `openWorkspaceThroughService`
 *    materializes it through the service port — the backend engine for
 *    the HTTP binding);
 *  - `operationIdentityOf` (the contract's identity derivation, used by
 *    the equivalence tests — a Node-context proof helper).
 */
import {
...
export interface WorkspaceClock {
  ...
  /**
   * The clock's SERIALIZABLE stepped spec (base + step) — the wire shape
   * the service port's revision leg carries (a function cannot cross the
   * HTTP boundary; the bindings rebuild it through the engine's own
   * steppedMaterializeClock arithmetic — the SAME numbers, never a second
   * clock semantics). Every workspace clock is a stepped clock by the
   * module's determinism law.
   */
  readonly spec: Readonly<{ readonly base: string; readonly stepMs: number }>;
}
...
export function steppedWorkspaceClock(base: string, stepMs: number): WorkspaceClock {
  ...
  return {
    now: () => iso(baseMs + ++ticks * stepMs),
    materializeAt: (stateIndex) => iso(baseMs + stateIndex * stepMs),
    spec: { base, stepMs },
  };
}
...
/** The default unit of a parameter slot, PER OPERATION TYPE (presentation
 *  convention mirroring the committed corpus's real-world wording: m for
 *  construction dimensions, mm for coats and pipe diameters). */
const PARAMETER_UNITS: Readonly<Record<string, Readonly<Record<string, string>>>> = Object.freeze({
  excavation: { depth: "m", width: "m", length: "m" },
  backfill: { depth: "m", width: "m", length: "m" },
  "demolition-removal": { length: "m", height: "m", thickness: "m" },
  "foundation-placement": { length: "m", width: "m", depth: "m", material: "" },
  "slab-placement": { length: "m", width: "m", thickness: "m", material: "" },
  "block-wall-placement": { length: "m", height: "m", thickness: "m", material: "" },
  "opening-creation": { width: "m", height: "m", material: "" },
  "plaster-application": { thickness: "mm", material: "" },
  "finish-application": { thickness: "mm", material: "" },
  "building-service-installation": { length: "m", diameter: "mm", material: "" },
} as const);

/** The default unit of one parameter of one operation type ("" = none). */
function parameterUnitOf(operationType: string, name: string): string {
  return PARAMETER_UNITS[operationType]?.[name] ?? "m";
}
  ...
  for (const operationType of types) {
    ...
    actions.push({
      ...
      parameterFields: entry.requiredParameters.map((name) => ({
        ...
        unit: MATERIAL_PARAMETERS.has(name) ? "" : parameterUnitOf(operationType, name),
...
> = Object.freeze({
  ...
  "face-set": ["demolition-removal", "plaster-application", "finish-application"],
  "surface-region": ["demolition-removal", "plaster-application", "finish-application"],
...
/** The open-workspace input: the case/solution context + the
 *  ENGINE-COMPUTED baseline overlay (layer 0). */
export interface OpenWorkspaceInput {
  ...
  /** The engine-materialized baseline overlay — injected, never derived
   *  client-side (the Node facade computes it through the local engine;
   *  `openWorkspaceThroughService` asks the service port — the backend
   *  engine for the HTTP binding). */
  readonly baselineState: ProposedState;
}

/**
 * Opens the workspace over an ENGINE-COMPUTED baseline overlay (layer 0)
 * and records the `inspect` step. Version 1 starts as an empty draft over
 * the pinned observed reality. The baseline state is an INJECTED
 * engine output — this module never derives identities itself (the
 * browser-safe cut: the state identity derivations need `node:crypto`).
 */
export function openWorkspace(input: OpenWorkspaceInput): SolutionWorkspaceState {
...
/**
 * Opens the workspace THROUGH THE SERVICE PORT (PROD-031): materializes
 * the baseline overlay (layer 0) via `deps.service.baseline` — the local
 * binding calls the engine directly, the HTTP binding posts
 * `POST /v1/solutions/baseline` — then opens over the returned
 * ENGINE-COMPUTED state. The browser mount's opening path (the async
 * leg of the workspace's mount lifecycle).
 */
export async function openWorkspaceThroughService(
  input: Omit<OpenWorkspaceInput, "baselineState">,
  deps: Pick<WorkspaceDeps, "service">,
): Promise<SolutionWorkspaceState> {
  const baseline = await deps.service.baseline({
    solutionId: input.solutionId,
    versionNumber: 1,
    baselineRealityVersionId: input.baselineRealityVersionId,
    materializedAt: input.materializedAt,
  });
  return openWorkspace({ ...input, baselineState: baseline.state });
}
...
export async function submitIntent(
  ...
  if (result.result.outcome === "applied") {
    return foldApplied(state, result.result, intent);
  }
...
export async function reviseOperation(
  ...
  const result = await deps.service.revise({
    ...
    revertOperationId,
    createdAt,
    materializeClock: deps.clock.spec,
    revisionProvenance: {
...
export async function applyAgentDecision(
  ...
  switch (decision.decision) {
    ...
    case "dispatch-operation": {
      ...
      const refused = next.notice !== undefined && next.notice.kind === "engine-refusal";
      // The submission path already recorded the journey step (confirm or
      // direct-manipulation + refusal detail); stamp the utterance onto it
      // so the trace joins the agent turn to the applied operation.
      const journey = [...next.journey];
      const lastIndex = journey.length - 1;
      const last = journey[lastIndex];
      if (last !== undefined) {
        journey[lastIndex] = { ...last, utterance };
      }
      return {
        ...next,
        journey,
...
async function applyAgentToolCommand(
  ...
  switch (command.kind) {
    ...
    case "explain": {
      ...
      text = `Step ${command.operationIndex} is a '${operation.operationType}' authored by ${operation.provenance.authoredBy}; the engine recorded ${operation.effects.length} effect(s).`;
      next = {
        ...state,
...
/* ------------------------------------------------------------------ */
/* The identity equivalence helper (the convergence-law proof) lives in */
/* the Node-side facade `./operations` (the contract's identity         */
/* derivation needs node:crypto — a Node-context proof helper, never    */
/* part of the browser graph).                                         */
/* ------------------------------------------------------------------ */
